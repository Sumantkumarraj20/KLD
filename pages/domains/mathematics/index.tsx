...
const DOMAIN: GameDomain = "mathematics";

export default function MathematicsDomainPage() {
  ...
  return (
    <>
      <Head>
        <title>Mathematics - Kids Points</title>
        <meta name="description" content="Practice Mathematics Skills" />
      </Head>
        ...
        <div className="max-w-6xl mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {Array.from({ length: 50 }, (_, i) => i + 1).map((level) => (
              <MathematicsLevelCard key={level} kidId={kid.kid_id} levelNumber={level} />
            ))}
          </div>
        </div>
      </div>
...
function MathematicsLevelCard({
  ...
  useEffect(() => {
    const unlockTime = gameAPI.getTimeUntilUnlock(kidId, "mathematics", levelNumber);
    const lockStatus = gameAPI.getLevelLockStatus(kidId, "mathematics", levelNumber);
    const levelCompletion = gameAPI.getLevelCompletion(kidId, "mathematics", levelNumber);
  ...
  const handleStart = () => {
    window.location.href = `/game?domain=mathematics&level=${levelNumber}`;
  };
  ...
  return (
    ...
    >
      ...
      {/* Level Number */}
      <div className="text-3xl font-bold text-purple-600 mb-2">Level {levelNumber}</div>
